...
4. 下側のペインで、パケットの 16 進データまたはデコードされたフィールド値を
  編集します。
...
- **Complete** のパケットを選択すると、下側のペインにペイロードが表示され、編集
  できます。上側のペインのすべての列は読み取り専用です。
...
下側のペインには、選択した完全なパケットのペイロードが表示され、編集できます。
一致するペイロード構造体定義がない場合、ヘッダーには **Payload: Hex only** と
表示されます。**hex** 列には、その行に表示されているバイト数と同じ長さの 16 進数を
入力します。**Bytes/row** では、ペイロードを一行にまとめて表示するバイト数を
変更できます。このモードでは、ほかの列は読み取り専用です。

定義が一致した場合は、最初に一致した定義を使ってペイロードがデコードされます。
**value** 列のフィールドを編集し、Enter キーを押すかフォーカスを移動すると値が
反映されます。ペイロードが再エンコードされ、キャプチャのバイト列が更新された後、
パケットが再度デコードされます。このモードでは **hex** 列とネストした構造体の行は
読み取り専用です。ほかの value 列以外の列も読み取り専用です。
