...
APP_PROTOCOL = "structlayouttoolkit"
...
def is_packaged() -> bool:
    ...
    if result == APPMODEL_ERROR_NO_PACKAGE:
        return False
    return result in (0, ERROR_INSUFFICIENT_BUFFER)
...
def start_editor_process(
    editor_name: EditorName, ) -> subprocess.Popen[bytes] | None:
    """Start an editor independently from the launcher process."""
    if is_packaged():
        os.startfile(f"{APP_PROTOCOL}:{editor_name}")
        return None
    if os.name == "nt":
...
def open_binary_editor(_parent: tk.Misc) -> subprocess.Popen[bytes] | None:
    """Open sltgui's binary editor in a separate process."""
    return start_editor_process("binary")


def open_packet_data_editor(_parent: tk.Misc) -> subprocess.Popen[bytes] | None:
...
def create_launcher(root: tk.Tk) -> ttk.Frame:
    ...
    actions: tuple[tuple[str, Callable[[tk.Misc],
...
def main() -> None:
    ...
    parser.add_argument("activation_uri", nargs="?")
    args = parser.parse_args()
    editor_name = args.editor
    if args.activation_uri is not None:
        protocol, separator, value = args.activation_uri.partition(":")
        if protocol != APP_PROTOCOL or not separator or value not in (
                "binary",
                "packet",
        ):
            parser.error("invalid activation URI")
        editor_name = value
    if editor_name is not None:
        run_editor(editor_name)
        return
