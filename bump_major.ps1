# bump_major.ps1 - Increment the major (1st) version number and reset minor/patch
#
# Usage:
#   .\bump_major.ps1
#
...
#   src/struct_layout_toolkit_launcher/_version.py
...
$newVer  = ('{0}.{1}.{2}'   -f $newMajor, $newMinor, $newPatch)
$oldVerW = ('{0}.{1}.{2}.0' -f $major, $minor, $patch)
$newVerW = ('{0}.{1}.{2}.0' -f $newMajor, $newMinor, $newPatch)
...
# __version__.py
$vpy     = Join-Path $root 'src\struct_layout_toolkit_launcher\_version.py'
...
Write-Host '  Updated: src/struct_layout_toolkit_launcher/_version.py'
