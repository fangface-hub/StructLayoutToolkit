...
def test_start_editor_process_detaches_from_launcher(monkeypatch):
    process = object()
    calls = []
    monkeypatch.setattr(app, "is_packaged", lambda: False)
...
def test_start_editor_process_uses_protocol_when_packaged(monkeypatch):
    calls = []
    monkeypatch.setattr(app, "is_packaged", lambda: True)
    monkeypatch.setattr(app.os, "startfile", calls.append)
    monkeypatch.setattr(
        app.subprocess,
        "Popen",
    ...
    result = app.start_editor_process("binary")

    assert result is None
    assert calls == ["structlayouttoolkit:binary"]
...
def test_main_runs_editor_from_protocol_activation(monkeypatch):
    calls = []
    monkeypatch.setattr(
        app.sys,
        "argv",
        ["launcher", "structlayouttoolkit:binary"],
    )
    monkeypatch.setattr(app, "run_editor", calls.append)
    monkeypatch.setattr(
        app.tk,
